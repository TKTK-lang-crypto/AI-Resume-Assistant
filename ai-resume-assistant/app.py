...
if match_btn:
    if check_inputs(require_resume=True):
        prompt = build_match_score_prompt(st.session_state.resume_text, st.session_state.job_description)
        safe_call_llm(prompt, "match_result", "match", "匹配度评分完成！")

if gap_btn:
    if check_inputs(require_resume=True):
        prompt = build_gap_analysis_prompt(st.session_state.resume_text, st.session_state.job_description)
        safe_call_llm(prompt, "gap_result", "gap", "技能缺口分析完成！")

if optimize_btn:
    if check_inputs(require_resume=True):
        prompt = build_resume_optimization_prompt(st.session_state.resume_text, st.session_state.job_description)
        safe_call_llm(prompt, "optimization_result", "optimize", "简历优化建议生成完成！")

if interview_btn:
    if check_inputs(require_resume=True):
        prompt = build_interview_prompt(st.session_state.resume_text, st.session_state.job_description)
        safe_call_llm(prompt, "interview_result", "interview", "模拟面试题生成完成！")

if full_btn:
    if check_inputs(require_resume=True):
        prompt = build_full_analysis_prompt(st.session_state.resume_text, st.session_state.job_description, analysis_mode)
